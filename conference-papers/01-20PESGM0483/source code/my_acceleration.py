...
def Contingency_Screening(Para, hour):
    ...
    if model.status == GRB.Status.OPTIMAL:
        sol = Result_Flow(model, Para, sign, y_line, f_line, c_line, v_flow)
        draw = Drawing(Para)
        draw.Figuring(Para, sol)
        return sol
    if model.status == GRB.Status.INFEASIBLE:
        # Severe contingency set
        sol = model._s_line
        return sol
...
# to eliminate "mild" N-1 contingency
#
...
if __name__ == "__main__":
    ...
    if mode == 0:
        ...
        Save_Result(Para, sol, "result/result_callback_33.xlsx")
    ...
    if mode == 1:
        ...
        t1   = time.perf_counter()
        print("Time used: %.2f" % (t1-t0), "s")
        Save_Result(Para, sol, "result/result_callback_69.xlsx")
    ...
    if mode == 3:
        ...
        Save_Time(Para, time_array, "result/result_callback_time.xlsx")